...
<?php
   ...
   $res=mysqli_query($conn_dtb, "SELECT DATE_FORMAT(ph.data, '%d.%m.%Y'), ph.TMOD, COUNT(ph.TMOD), SUM(if(ph.inchis = 1 AND ph.cursv > 1, ph.pret_km * ph.cursv, if(ph.inchis = 1, ph.pret_km * 4.8, ph.pret_km))) FROM plan_hdr ph WHERE ph.data = '".$dt."' AND ph.tr_id <> 9 AND ph.inchis <> 9 GROUP BY ph.TMOD ORDER BY ph.TMOD") or die(mysql_error());
   ...
   while (isset($row[$i][0])){
	  ...
	  $tlivr+=$row[$i][2];
	  $res=mysqli_query($conn_dtb, "SELECT SUM(if(LOCATE('E', moneda)=1 ,cant * pret * 4.8, if(LOCATE('E', moneda)=1, cant * pret * 4.8, if(LOCATE('OFL', moneda)=1, cant * pret * 4.8, if(LOCATE('OFR', moneda)=1, cant * pret * 5)))) FROM plan, plan_hdr WHERE plan_hdr.id=pid AND plan_hdr.TMOD = '".$row[$i][1]."' AND plan_hdr.data='".$dt."' AND plan_hdr.tr_id <> 9 AND plan_hdr.inchis <> 9");
...
<?php
   ...
   if ($row[15] == 6)
	   ...
	   echo "<td align='left' width='220'>".$row[20]."</a></td>";
